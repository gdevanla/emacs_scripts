...
(add-hook 'python-mode-hook
          (lambda ()
                ...
                (set (make-local-variable 'ac-sources) '(ac-source-rope))
...
;;   (interactive)
;;     (setq x (cdr(split-string (thing-at-point 'sentence) "[?\(?\,?\:?\?\ )]"
;;     )))
;;     (forward-line 1)
;;     (insert "\t\"\"\"\n\n\n")
;;     (while x
;;     (if (/= (length (chomp (car x))) 0)
;;     (progn
;;         (insert "\t:param ")
;;         (insert (chomp (car x)))
;;         (insert ": \n")))
...
;; settings from http://ergoemacs.org/emacs/emacs_editing_lisp.html
